...
class UserService {
  ...
  public async findUserByName(userName: string): Promise<UserRetrive | null> {
    const query = 'SELECT UserId, UserName, email, Password FROM userdetails WHERE email = ? OR mobileNo = ?';
  
    // Cast the result to an array of RowDataPacket
    const [rows] = await this.db.execute<RowDataPacket[]>(query, [userName, userName]);
